...
class Client:
    class _Contact:
        ...
        def __init__(self, name: str, login: str):
            if _Contact.database is None:
                raise Exception("You shold initialise Contact.database first!")
            self._name = name
            self._login = login
            self._ip = _Contact.database.search_ip(name)
            history = _Contact.database.search_messages(name)
        ...
        def add_text_message(self, message: str):
            _Contact.database.add_message(self._name, datetime.now(), True, message)
        ...
        def upgrade_ip(self, new_ip: str):
            _Contact.database.update_ip(self._name, new_ip)
            self._ip = new_ip
    ...
    async def __init__(self, login: str, password: str,
        ...
        def _add_new_contact(self):
            def _add_new_contact_wrapped(name: str, ip: str):
                ...
                return _Contact(name, self._login)
            return _add_new_contact_wrapped
        
        self._contacts = _Contact_dict(_add_new_contact(self))
