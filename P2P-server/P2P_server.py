...
async def _get_data(reader : asyncio.StreamReader, timeout : int = 10) -> (int, bytes):
    async def _l_get_data(len : int):
        data = await asyncio.wait_for(reader.read(len), timeout=timeout)
        if data:
            return data
        else:
            raise ConnectionAbortedError()
    len = await _l_get_data(1)
    result = await _l_get_data(int(len[0]))
    return (int(result[0]), result[1:])

async def _send_data(writer : asyncio.StreamReader, code : int, data : bytes):
...
async def _handle_connection(reader : asyncio.StreamReader, writer : asyncio.StreamWriter):
    ...
    while True:
        try:
            code, data = await _get_data(reader)
...
if __name__ == "__main__":
    ...
    server_gen = asyncio.start_server(_handle_connection, port=3501)
